...
COUNTRY_NAMES = {
    "US": "United States", "EG": "Egypt", "ZA": "South Africa", "GR": "Greece", "NL": "Netherlands",
    "BE": "Belgium", "FR": "France", "ES": "Spain", "HU": "Hungary", "IT": "Italy",
    "RO": "Romania", "CH": "Switzerland", "AT": "Austria", "GB": "United Kingdom", "DK": "Denmark",
    "SE": "Sweden", "NO": "Norway", "PL": "Poland", "DE": "Germany", "PE": "Peru",
    "MX": "Mexico", "CU": "Cuba", "AR": "Argentina", "BR": "Brazil", "CL": "Chile",
    "CO": "Colombia", "VE": "Venezuela", "MY": "Malaysia", "AU": "Australia", "ID": "Indonesia",
    "PH": "Philippines", "NZ": "New Zealand", "SG": "Singapore", "TH": "Thailand", "RU": "Russia",
    "JP": "Japan", "KR": "South Korea", "VN": "Vietnam", "CN": "China", "TR": "Turkey",
    "IN": "India", "PK": "Pakistan", "AF": "Afghanistan", "LK": "Sri Lanka", "MM": "Myanmar",
    "IR": "Iran", "SS": "South Sudan", "MA": "Morocco", "DZ": "Algeria", "TN": "Tunisia",
    "LY": "Libya", "GM": "Gambia", "SN": "Senegal", "MR": "Mauritania", "ML": "Mali",
    "GN": "Guinea", "CI": "Ivory Coast", "BF": "Burkina Faso", "NE": "Niger", "TG": "Togo",
    "BJ": "Benin", "MU": "Mauritius", "LR": "Liberia", "SL": "Sierra Leone", "GH": "Ghana",
    "NG": "Nigeria", "TD": "Chad", "CF": "Central African Republic", "CM": "Cameroon", "CV": "Cape Verde",
    "ST": "Sao Tome and Principe", "GQ": "Equatorial Guinea", "GA": "Gabon", "CG": "Republic of the Congo", "CD": "DR Congo",
    "AO": "Angola", "GW": "Guinea-Bissau", "IO": "British Indian Ocean Territory", "SC": "Seychelles", "SD": "Sudan",
    ...
    "PT": "Portugal", "LU": "Luxembourg", "IE": "Ireland", "IS": "Iceland", "AL": "Albania",
    "MT": "Malta", "CY": "Cyprus", "FI": "Finland", "BG": "Bulgaria", "LT": "Lithuania",
    "LV": "Latvia", "EE": "Estonia", "MD": "Moldova", "AM": "Armenia", "BY": "Belarus",
    "AD": "Andorra", "MC": "Monaco", "SM": "San Marino", "VA": "Vatican City", "UA": "Ukraine",
    "RS": "Serbia", "ME": "Montenegro", "XK": "Kosovo", "HR": "Croatia", "SI": "Slovenia",
    "BA": "Bosnia and Herzegovina", "MK": "North Macedonia", "CZ": "Czech Republic", "SK": "Slovakia", "LI": "Liechtenstein",
    "FK": "Falkland Islands", "BZ": "Belize", "GT": "Guatemala", "SV": "El Salvador", "HN": "Honduras",
    "NI": "Nicaragua", "CR": "Costa Rica", "PA": "Panama", "PM": "Saint Pierre and Miquelon", "HT": "Haiti",
    "GP": "Guadeloupe", "BO": "Bolivia", "GY": "Guyana", "EC": "Ecuador", "GF": "French Guiana",
    "PY": "Paraguay", "MQ": "Martinique", "SR": "Suriname", "UY": "Uruguay", "BQ": "Caribbean Netherlands",
    "TL": "Timor-Leste", "BN": "Brunei", "NR": "Nauru", "PG": "Papua New Guinea", "TO": "Tonga",
    "SB": "Solomon Islands", "VU": "Vanuatu", "FJ": "Fiji", "PW": "Palau", "WF": "Wallis and Futuna",
    "CK": "Cook Islands", "NU": "Niue", "WS": "Samoa", "KI": "Kiribati", "NC": "New Caledonia",
    "TV": "Tuvalu", "PF": "French Polynesia", "TK": "Tokelau", "FM": "Micronesia", "MH": "Marshall Islands",
    "KP": "North Korea", "HK": "Hong Kong", "MO": "Macau", "KH": "Cambodia", "LA": "Laos",
    "BD": "Bangladesh", "TW": "Taiwan", "MV": "Maldives", "LB": "Lebanon", "JO": "Jordan",
    "SY": "Syria", "IQ": "Iraq", "KW": "Kuwait", "SA": "Saudi Arabia", "YE": "Yemen",
    "OM": "Oman", "PS": "Palestine", "AE": "United Arab Emirates", "IL": "Israel", "BH": "Bahrain",
    "QA": "Qatar", "BT": "Bhutan", "MN": "Mongolia", "NP": "Nepal", "TJ": "Tajikistan",
    "TM": "Turkmenistan", "AZ": "Azerbaijan", "GE": "Georgia", "KG": "Kyrgyzstan", "UZ": "Uzbekistan",
    "BS": "Bahamas", "BB": "Barbados", "AI": "Anguilla", "AG": "Antigua and Barbuda", "VG": "British Virgin Islands",
    "VI": "US Virgin Islands", "BM": "Bermuda", "GD": "Grenada", "TC": "Turks and Caicos", "MS": "Montserrat",
    "GU": "Guam", "AS": "American Samoa", "SX": "Sint Maarten", "LC": "Saint Lucia", "DM": "Dominica",
    "VC": "Saint Vincent and the Grenadines", "DO": "Dominican Republic", "TT": "Trinidad and Tobago",
    "KN": "Saint Kitts and Nevis", "JM": "Jamaica", "PR": "Puerto Rico",
}
...
def send_admin_panel(chat_id, message_id=None):
    ...
    if message_id:
        bot.edit_message_text(text, chat_id=chat_id, message_id=message_id, parse_mode="Markdown", reply_markup=admin_panel_markup())
    else:
        bot.send_message(chat_id, text, parse_mode="Markdown", reply_markup=admin_panel_markup())
...
def country_list_markup(service_name, callback_prefix):
    """একটা সার্ভিসের ভেতরের দেশগুলোর লিস্ট বাটন আকারে দেখায় (পুরো নাম সহ)"""
    markup = types.InlineKeyboardMarkup()
    countries = services.get(service_name, {})
    for iso in countries.keys():
        flag = next((f for f, i in COUNTRY_FLAGS.values() if i == iso), "🌍")
        markup.add(types.InlineKeyboardButton(f"{flag} {country_full_name(iso)}", callback_data=f"{callback_prefix}|{service_name}|{iso}"))
    return markup
...
def admin_add_service(call):
    ...
    bot.edit_message_text(
        "⌨️ নতুন সার্ভিসের নাম লিখুন (যেমন: Facebook):",
        chat_id=call.message.chat.id,
        message_id=call.message.message_id
    )
    admin_waiting[ADMIN_ID] = {"type": "add_service", "msg_id": call.message.message_id}
...
def admin_remove_service(call):
    ...
    bot.answer_callback_query(call.id)
    if not services:
        bot.edit_message_text("ℹ️ কোনো সার্ভিস নেই।", chat_id=call.message.chat.id, message_id=call.message.message_id)
        return
    bot.edit_message_text(
        "🗑 কোন সার্ভিসটা মুছবেন?",
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        reply_markup=service_list_markup("rmsvc")
    )
...
def handle_remove_service(call):
    ...
    bot.edit_message_text(
        f"✅ সার্ভিস **{name}** (সব দেশ/রেঞ্জসহ) সম্পূর্ণ মুছে ফেলা হয়েছে।",
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        parse_mode="Markdown"
    )
...
def admin_set_range(call):
    ...
    if not services:
        bot.edit_message_text(
            "ℹ️ আগে **Add Service** দিয়ে একটা সার্ভিস তৈরি করুন।",
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            parse_mode="Markdown"
        )
        return
    bot.edit_message_text(
        "🎯 কোন সার্ভিসে নতুন দেশ/রেঞ্জ যোগ করবেন?",
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        reply_markup=service_list_markup("setrange")
    )
...
def handle_select_service_for_range(call):
    ...
    admin_waiting[ADMIN_ID] = {"type": "set_range", "service": name, "msg_id": call.message.message_id}
...
def admin_remove_range(call):
    ...
    if not services:
        ...
        return
    bot.edit_message_text(
        "❌ কোন সার্ভিস থেকে রেঞ্জ মুছবেন?",
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        reply_markup=service_list_markup("rmrangesvc")
    )
...
def handle_select_service_for_remove_range(call):
    ...
    if not countries:
        bot.edit_message_text(
            f"ℹ️ **{name}** এ কোনো দেশ/রেঞ্জ নেই।",
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            parse_mode="Markdown"
        )
...
def handle_remove_range(call):
    ...
    bot.answer_callback_query(call.id, text=f"✅ {name} থেকে {country_full_name(iso)} মুছে ফেলা হয়েছে")
    bot.edit_message_text(
        f"✅ **{name}** থেকে **{country_full_name(iso)}** এর রেঞ্জ মুছে ফেলা হয়েছে।",
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        parse_mode="Markdown"
    )
...
def handle_text(message):
    ...
    if user_id == ADMIN_ID and user_id in admin_waiting:
        ...
        if state["type"] == "set_range":
            ...
            bot.edit_message_text(
                f"✅ **{name}** এ {flag} **{country_full_name(iso)}** যোগ/আপডেট হয়ে গেছে — রেঞ্জ: `{rid_input}`",
                chat_id=message.chat.id,
                message_id=prompt_msg_id,
                parse_mode="Markdown"
            )
...
# ──────────────────────────────────────────────────────────
# 📱 ইউজার একটা সার্ভিস বেছে নিলে — সেই একই মেসেজ এডিট হয়ে দেশ লিস্ট দেখাবে
# ──────────────────────────────────────────────────────────
...
def handle_get_service_countries(call):
    ...
    if not countries:
        bot.edit_message_text(
            f"❌ No country available for {name}.",
            chat_id=call.message.chat.id,
            message_id=call.message.message_id
        )
        return
...
# 🌍 ইউজার একটা দেশ বেছে নিলে — একই মেসেজ এডিট হয়ে নাম্বার আনা শুরু হয়
...
def handle_get_country_numbers(call):
    ...
    bot.edit_message_text(
        f"🔍 {service_emoji(name)} {name} ({country_full_name(iso)}) থেকে নাম্বার খোঁজা হচ্ছে...",
        chat_id=call.message.chat.id,
        message_id=call.message.message_id
    )
    threading.Thread(
        target=_fetch_and_send_numbers,
        args=(call.message.chat.id, user_id, rid_input, call.message.message_id, name, iso),
        daemon=True
    ).start()
...
def _fetch_and_send_numbers(chat_id, user_id, rid_input, message_id, service_name, country_iso):
    count = user_get_count.get(user_id, DEFAULT_GET_COUNT)
    try:
    ...
    if valid_results:
        ...
        bot.edit_message_text(result_text, chat_id=chat_id, message_id=message_id, parse_mode="Markdown", reply_markup=markup)
    else:
        bot.edit_message_text(
            "❌ No numbers available in this range right now. Try again later.",
            chat_id=chat_id,
...
def fetch_live_traffic_text():
    ...
    try:
        ...
        ranked_combos = sorted(combo_counts.items(), key=lambda x: x[1], reverse=True)
        for idx, ((iso, flag, sid), cnt) in enumerate(ranked_combos[:10], start=1):
            pct = round((cnt / total) * 100)
            lines.append(f"{idx}. {flag} {iso} — {pct}% ({sid})")
